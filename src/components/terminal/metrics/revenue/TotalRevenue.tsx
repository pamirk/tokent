...
const TotalRevenue = (props: Props) => {
  ...
  const chartData = useMemo(
    () =>
      labelRevenueMetrics(data, selectedLength, category, chartType, "revenue"),
    [data, selectedLength, chartType, category]
  )
  ...
  return (
    <ChartContainer
      title="Historical total revenue"
      tooltipId="chart-total-revenue"
      name={`historical-revenue-${category}`}
      embedUrl={
        window.location.origin + "/terminal/metrics/revenue/embed/historical"
      }
      url={window.location.origin + "/terminal/metrics/revenue/historical"}
