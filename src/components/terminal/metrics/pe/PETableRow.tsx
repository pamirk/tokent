  ...
  return (
    <div key={project.name}>
      <TableRowContainer onClick={() => setIsOpen((prevIsOpen) => !prevIsOpen)}>
        ...
        <TableRowColumn>
          {getLabelForPrice(project.revenue_protocol_annualized)}
        </TableRowColumn>
        <TableRowColumn>{getLabelForX(project.pe)}</TableRowColumn>
        <TableRowColumn style={getColor(reverseNumeral(project.pe_24h_change))}>
          {getLabelForPercentage(project.pe_24h_change)}
        </TableRowColumn>
        <TableRowColumn style={getColor(reverseNumeral(project.pe_7d_change))}>
          {getLabelForPercentage(project.pe_7d_change)}
        </TableRowColumn>
        <TableRowColumn style={getColor(reverseNumeral(project.pe_30d_change))}>
          {getLabelForPercentage(project.pe_30d_change)}
        </TableRowColumn>
        <TableRowColumn style={getColor(reverseNumeral(project.pe_90d_change))}>
          {getLabelForPercentage(project.pe_90d_change)}
        </TableRowColumn>
        <TableRowColumn
          style={getColor(reverseNumeral(project.pe_180d_change))}
        >
          {getLabelForPercentage(project.pe_180d_change)}
        </TableRowColumn>
      </TableRowContainer>
      {isOpen && !isMobile && <RowContents project={project} />}
    </div>
