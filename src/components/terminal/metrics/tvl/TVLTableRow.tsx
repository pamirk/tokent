  ...
  return (
    <div key={project.name}>
      <TableRowContainer onClick={() => setIsOpen((prevIsOpen) => !prevIsOpen)}>
        ...
        <TableRowColumn style={getColor(project.tvl_24h_change)}>
          {getLabelForPercentage(project.tvl_24h_change)}
        </TableRowColumn>
        <TableRowColumn style={getColor(project.tvl_7d_change)}>
          {getLabelForPercentage(project.tvl_7d_change)}
        </TableRowColumn>
        <TableRowColumn style={getColor(project.tvl_30d_change)}>
          {getLabelForPercentage(project.tvl_30d_change)}
        </TableRowColumn>
        <TableRowColumn style={getColor(project.tvl_180d_change)}>
          {getLabelForPercentage(project.tvl_180d_change)}
        </TableRowColumn>
        <TableRowColumn>{getLabelForPrice(project.tvl_30d)}</TableRowColumn>
      </TableRowContainer>
      {isOpen && !isMobile && <RowContents project={project} />}
    </div>
  )
