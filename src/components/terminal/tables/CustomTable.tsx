...
import OverviewTableRow from "components/home/overview/overviewTable/OverviewTableRow"
import { FavoriteTableType } from "components/account/favorites/Favorites"
...
const CustomTable = () => {
  document.title = "Token Terminal | Custom data table"
  ...
  const [tableName, setName] = useState("Projects")
  ...
  useEffect(() => {
    const favorites = user.config.favorites?.tables

    if (!favorites) return

    const currentSettings = favorites.find(
      (fav: FavoriteTableType) => fav.id === tableId
    )
    if (!currentSettings) return

    setCustomTableColumns(currentSettings.items)
